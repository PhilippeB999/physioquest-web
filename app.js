/* ============================================================
   PhysioQuest — logique de jeu (gamification)
   DEC Techniques de physiothérapie (144.A0)
   ============================================================ */

const STORAGE_KEY = "physioquest_state_v1";
...
const ACCESS_CODES = ["PHYSIO-2026-PBEL", "CSTJEAN-2026-PHYS"]; // PLACEHOLDERS — à finaliser avant vente. Client visé : Cégep Saint-Jean-sur-Richelieu.
...
const SYNC_KEY = "physioquest_sync_pending";
...
function deviceId() {
  let id = localStorage.getItem("physioquest_device_id");
  if (!id) {
    id = "pq-" + (crypto.randomUUID ? crypto.randomUUID() : Date.now().toString(36) + Math.random().toString(36).slice(2));
    localStorage.setItem("physioquest_device_id", id);
  }
  return id;
}
